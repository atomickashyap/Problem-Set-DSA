...
ll findmin(vll v, int n, ll tsum, ll current)
{
    if (n == 0)
    {
        return abs((tsum - current) - current);
    }

    return min(findmin(v, n - 1, tsum, current + v[n - 1]), findmin(v, n - 1, tsum, current));
}
